...
with customer_category as (
    select c.customer_id, fc.category_id
    from customer c
...
with actor_category as (
    select a.actor_id, fc.category_id
    from actor a
...
where f.rating = 'R' and not exists (
    select 1
    from film f2
    join film_actor fa2 on f2.film_id = fa2.film_id
    join actor a2 on fa2.actor_id = a2.actor_id
    where f2.rating = 'G' and a2.actor_id = a.actor_id
);
...
select a.first_name, a.last_name, c.name as category_name, avg(datediff(r.return_date, r.rental_date)) AS avg_rental_duration
from actor a
join film_actor fa on a.actor_id = fa.actor_id
join film f on fa.film_id = f.film_id
...
join film_actor fa on a.actor_id = fa.actor_id
join film f on fa.film_id = f.film_id
where f.rating = 'R' and f.length > 120 and not exists (
    select 1
    from film f2
    join film_actor fa2 on f2.film_id = fa2.film_id
    join actor a2 on fa2.actor_id = a2.actor_id
...
WITH ActorPairs AS (
    ...
    ActorCollaborations AS (
        ...
        FROM actor a
        LEFT JOIN (
            SELECT actor1_id, actor2_id FROM ActorPairs
            UNION ALL
            SELECT actor2_id, actor1_id FROM ActorPairs
        ) ap ON a.actor_id IN (ap.actor1_id, ap.actor2_id)
        GROUP BY a.actor_id, a.first_name, a.last_name
    )
    SELECT 
        ac.first_name,
            ...
            CONCAT(
                CASE 
                    WHEN ac.actor_id = ap.actor1_id THEN 
                        CONCAT(ap.actor2_first_name, ' ', ap.actor2_last_name, ' (', ap.films_together, ' films)')
                    ELSE 
                        CONCAT(ap.actor1_first_name, ' ', ap.actor1_last_name, ' (', ap.films_together, ' films)')
                END
            )
...
WITH CustomerCategoryRentals AS (
    ...
    SELECT 
        ...
        cwac.last_name,
        GROUP_CONCAT(
            CONCAT(ccr.category_name, ': ', ccr.films_rented, ' films')
            ORDER BY ccr.category_name
            SEPARATOR ', '
        ) as rental_breakdown
...
WITH CustomerCategories AS (
    ...
    SELECT DISTINCT
        c.first_name,
        c.last_name,
        cat.name as new_category_name,
        DATE(ftc.rental_date) as rental_date
    FROM FirstTimeCategories ftc
    JOIN customer c ON ftc.customer_id = c.customer_id
